...
def predict_another_case(tryDic,acqDic,relDic,start_time,end_time,classL,newClassL,n_items_l,namesD,a1,b1,a3,b3,serv,if_up): #serv has the service rates, not time
	predicted3 = []
	predicted1 = []
	...
	for n in n_items_l: #n is the number of items to be done
		if j < (len(serv)):
			...
			if if_up:
				serv[j][1][class1_idx] = 1.0/(a3*n+b3) #res_up[1] is the service time matrix
...
def pre_process(filename,n):
	...
	print "class2EndT:", class2EndT
	print "class3EndT:", class3EndT

	l = filter(lambda x: x!= [], relDic.values())
	startRel = minT(l,1)[1]
	endRel = maxT(l,1)[1]
	return tryDic,acqDic,relDic,namesD,start_time,end_time,(class1EndT,class2EndT,class3EndT),classList,classL
...
def get_polyfit(data_file):
	...
	lockid = 0
	print "analyzing 222"
	tryDic,acqDic,relDic,namesD,start_time,end_time, stage_end_time,classList,classL = pre_process(data_file,n)
	...
	serv_up1, items_up1 = get_n_items_and_serv(start_time,class2EndT,2,0,tryDic,acqDic,relDic,namesD,n,classList,lockid)
	serv_up2, items_up2 = get_n_items_and_serv(start_time,class2EndT,2,0,tryDic,acqDic,relDic,namesD,n,classList,lockid)

	(a1_up,b1_up) = polyfit(items_up,serv_up[class1_idx],1)
	(a3_up,b3_up) = polyfit(items_up,serv_up[class3_idx],1)

	print_list_vertical(items_up,"number of items up")
	print_list_vertical(serv_up[class1_idx],"service time class 1 up")
	print_list_vertical(serv_up[class3_idx],"service time class 3 up")
...
def get_derivative_analysis():
	...
	serv_l_up,m3_up,analyzed3_up,m1_up,analyzed1_up = slotted_analyzed(start_time,class1EndT,2,0,tryDic,acqDic,relDic,namesD,n_target,classList) #devide the part before class 1 threads finish into 2 parts
	serv_l_down,m3_down,a3_down,m1_down,a1_down = slotted_analyzed(class1EndT,end_time,0,slot_size,tryDic,acqDic,relDic,namesD,n_target,classList)
	print_list_vertical(m3_up + m3_down ,"measured3")
	print_list_vertical(analyzed3_up + a3_down ,"analyzed3")
	...
	if  if_predict:
		(a1_up,b1_up,a3_up,b3_up,a3_down,b3_down) = get_polyfit(file_base)

		uphill_classL = (10,10,10)
		n_items_l = [216, 654, 1068, 1495, 1981, 2499, 3063, 3667, 4243, 4728]
		print "predicting uphill:"
		p1_up,p3_up,p1_w_cont_up, p3_w_cont_up = predict_another_case(tryDic,acqDic,relDic,start_time,class1EndT,classL,uphill_classL,n_items_l,namesD,a1_up,b1_up,a3_down,b3_down,serv_l_up,1)
	...
	print "predicted1_up:"
	print p1_up
	print "predicted1 with contention up:"
	print p1_w_cont_up
